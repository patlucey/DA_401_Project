...
```{r}
duck_data2 <- read_csv('/Users/patrick/Documents/Denison/DA401/Proj/tidy_duck_data.csv')

robin_data <- read_csv('/Users/patrick/Documents/Denison/DA401/Proj/tidied_robin_data.csv')
...
# Read the image at its native resolution
image <- image_read('/Users/patrick/Documents/Denison/DA401/Proj/noise2.png')

# You can process the image if needed (resize, crop, etc.)

# Save the image with high resolution
image_write(image, '/Users/patrick/Documents/Denison/DA401/Proj/noise2.tif', format = 'tif')
```
...
```{r}
df6 <- data.frame(coordss,
                 Noise_1 = noise_1_values,
                 Noise_2 = noise_2_values,
                 Noise_3 = noise_3_values)
```
...
# Extract values from the raster at the location of points
extracted_values <- extract(raster_data1, points_vector)

# Assuming the points_grid has the same order as extracted_values, you can add these values to it
points_grid$Value <- extracted_values[,1]  # Assuming interest in the first layer

# Now, you can proceed with creating your interactive map with leaflet as before
leaflet() %>%
  addProviderTiles(providers$Esri.WorldImagery) %>%
  addCircleMarkers(
    data = points_grid,
    ~x, ~y,
    popup = ~paste("Urban Expansion Probability:", Value),
    radius = 1,
    color = "#FF5733",
    fillOpacity = 0.8
  ) %>%
...
```{r}
 raster_df2 <- raster_df2 %>% rename(Probabilty = rastergoodes)
...
output_file_path <- "~/Desktop/duck_sf.csv"

# Export the data frame to the CSV file
write.csv(duck_data_sf, output_file_path, row.names = FALSE)
...
```{r}
joined_sf_direct <- st_join(goose_data_sf, raster_df_sf, join = st_intersects)
```

```{r}
# Add these indices as a new column in duck_data_sf
duck_data_sf$nearest_raster_idx <- nearest_indices
```

```{r}
# Join the nearest raster data to duck_data_sf
# This requires a little manipulation since nearest_indices are numeric indices
# relating to the row numbers of raster_df_sf
duck_data_sf <- duck_data_sf %>%
  st_join(raster_df_sf %>% mutate(raster_idx = row_number()), 
            by = c("nearest_raster_idx" = "raster_idx"))
```

```{r}
unique(merged_data$Probabilty)
...
```{r}
model <- lm(longitude ~ Probabilty + latitude
, data = merged_data)
```
...
```{r}
raster_df3 <- as.data.frame(raster_noise, xy=TRUE)
raster_df3
```
...
```{r}
# Ensure values are correctly extracted as a vector
...
# Preview the first few coordinates
head(cellCoords)
```
...
ggplot(duck_data, aes(x = `individual-local-identifier`)) +
  geom_histogram(stat = "count", fill = "skyblue", color = "black") + # Specify binwidth if necessary
  theme_minimal(base_size = 14) + # Use a minimal theme for a clean look
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), # Improve label readability
        plot.title = element_text(hjust = 0.5, size = 16), # Center and size the title
        plot.subtitle = element_text(hjust = 0.5, size = 12), # Center and size the subtitle
...
```{r}
# Assuming duck_data is available and contains columns for `individual-local-identifier`, `location-long`, `location-lat`, and `timestamp`
...
map2 <- leaflet(specific_duck2) %>%
  addProviderTiles(providers$Esri.WorldImagery) %>%  # Use Esri.WorldImagery for base tiles
  addCircleMarkers(~`longitude`, ~`location-lat`, 
                   popup = ~as.character(timestamp), 
                   color = "#0078A8", fillColor = "#0078A8", 
                   fillOpacity = 0.8, weight = 1, radius = 5) %>%  # Customized markers
...
# Set the initial view centered over North America
map_combined <- map_combined %>%
  setView(lng = -95.7129, lat = 37.0902, zoom = 4)
