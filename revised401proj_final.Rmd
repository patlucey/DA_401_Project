...
```{R}
# Establishes the North American bounding
north_america_extent <- ext(-169, -52, 7, 84)
```

```{r}
# converts raster_data1 into the proper format
...
### Visualizing the migration routes
```{r}
...
```
### Next 50 lines is code to establish a merged object
```{r}
...
### Updated chunks below:
```{r}
# 
t_test_result <- t.test(filtered_sf_direct2$Urban_prob, filtered_sf_direct$Urban_prob, var.equal = FALSE) # Use var.equal = FALSE for Welch's t-test
...
```{r}
# Filtering NAs from each dataset before combining
filtered_sf_direct <- joined_sf_direct %>% filter(!is.na(Urban_prob))
filtered_sf_direct2 <- joined_sf_direct2 %>% filter(!is.na(Urban_prob))

```


```{r}
# Combine the data from both filtered data frames for plotting
data_for_plot <- data.frame(
  value = c(filtered_sf_direct$Urban_prob, filtered_sf_direct2$Urban_prob),
  group = factor(rep(c("Red Throated Loon", "Canadian Goose"), c(length(filtered_sf_direct$Urban_prob), length(filtered_sf_direct2$Urban_prob))))
)

# Create a violin plot to visualize the distributions of the two groups
p <- ggplot(data_for_plot, aes(x = group, y = value, fill = group)) +
  geom_violin(alpha = 0.3, trim = TRUE) +  # Use violin plot instead of box plot
  stat_summary(fun = mean, geom = "point", shape = 20, size = 3, color = "red") +  # Display mean as red point
  labs(title = "Violin Plot of Associated Urbanization levels by species location",
       x = "Species",
       y = "Urbanization level") +
  theme_minimal() +
  scale_fill_brewer(palette = "Pastel2")  # Color differentiation
